...
namespace WeifenLuo.WinFormsUI.Docking
{
    ...
    public partial class DockPane : UserControl, IDockDragSource
    {
        ...
        private void InternalConstruct(IDockContent content, DockState dockState, bool flagBounds, Rectangle floatWindowBounds, DockPane prevPane, DockAlignment alignment, double proportion, bool show)
        {
            ...
            this.m_splitter = content.DockHandler.DockPanel.Extender.DockPaneSplitterControlFactory.CreateSplitterControl(this);
		...
		internal void TestDrop( DockHelper.CursorPoint info, DockOutlineBase dockOutline )
        {
			...
			HitTestResult hitTestResult = this.GetHitTest( info.Cursor);
        ...
        {
            this.m_countRefreshStateChange--;
            System.Diagnostics.Debug.Assert(this.m_countRefreshStateChange >= 0);
            this.DockPanel.ResumeLayout(true, true);
        }
        ...
        protected override void OnParentChanged(EventArgs e)
        {
            ...
            var newParent = this.Parent as DockWindow;
            if (newParent != this._lastParentWindow)
            {
                if (this._lastParentWindow != null)
                    this._lastParentWindow.PerformLayout();
                this._lastParentWindow = newParent;
            }
        }
        #endregion
    }
